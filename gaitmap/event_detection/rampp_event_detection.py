...
def _detect_min_vel(gyr: np.ndarray, min_vel_search_win_size: int) -> float:
    ...
    if min_vel_search_win_size >= len(energy):
        raise ValueError("Either the value chosen for min_vel_search_win_size_ms is too large (should be 100 ms)"                      f"or the passed data of gyr is too short (it has length {len(gyr)}).")
