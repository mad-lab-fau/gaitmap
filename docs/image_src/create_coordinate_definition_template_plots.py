...
dataset_sf = sensor_alignment.align_dataset_to_gravity(
    example_dataset, sampling_rate_hz
)
...
def plot_stride(data, column_names, sensor_id, stride_id, export_name):
    ...
    for i, col in enumerate(column_names[3:]):
        axs[0].plot(
            data[sensor_id][col].to_numpy()[start:end], label=col, color=colors[i]
        )
...
plot_stride(
    dataset_sf, SF_COLS, "left_sensor", 5, "left_sensor_sensor_frame_template.pdf"
)
plot_stride(
    dataset_sf, SF_COLS, "right_sensor", 18, "right_sensor_sensor_frame_template.pdf"
)

#%%
# Plot "Stride-Template" in Body Frame
plot_stride(
    dataset_bf, BF_COLS, "left_sensor", 5, "left_sensor_body_frame_template.pdf"
)
plot_stride(
    dataset_bf, BF_COLS, "right_sensor", 18, "right_sensor_body_frame_template.pdf"
)
