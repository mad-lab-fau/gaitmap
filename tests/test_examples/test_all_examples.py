...
def test_barth_dtw_example(snapshot) -> None:
...
def test_barth_dtw_custom_template(snapshot) -> None:
...
def test_constrained_barth_dtw_example(snapshot) -> None:
...
def test_preprocessing_example(snapshot) -> None:
...
def test_sensor_alignment_detailed_example(snapshot) -> None:
    ...
    for sensor in ["left_sensor", "right_sensor"]:
        snapshot.assert_match(gravity_aligned_data[sensor].to_numpy()[:1000], f"{sensor}_gravity")
        snapshot.assert_match(pca_aligned_data[sensor].to_numpy()[:1000], f"{sensor}_pca")
        snapshot.assert_match(forward_aligned_data[sensor].to_numpy()[:1000], f"{sensor}_forward")


def test_sensor_alignment_compact(snapshot) -> None:
    from examples.preprocessing.automatic_sensor_alignment_compact import (
        forward_aligned_data,
    )
...
def test_spatial_parameters(snapshot) -> None:
    from examples.parameters.spatial_parameters import p

    snapshot.assert_match(p.parameters_["left_sensor"])


def test_rampp_event_detection(snapshot) -> None:
...
def test_json_example(snapshot) -> None:
...
def test_trajectory_reconstruction(snapshot) -> None:
...
def test_region_trajectory_reconstruction(snapshot) -> None:
...
def test_ullrich_gait_sequence_detection(snapshot) -> None:
...
def test_caching(snapshot) -> None:
...
def test_grid_search(snapshot) -> None:
...
def test_advanced_kalman(snapshot) -> None:
