...
class TestForwardBackwardIntegration:
    ...
    def test_estimate_dummy_data(self):
        ...
        dummy_pd = pd.DataFrame(data=np.tile(dummy_data, 6).reshape(len(dummy) * 2, 6), columns=SF_COLS)
        sampling_frequency_hz = 100

        position = ForwardBackwardIntegration(turning_point, steepness)
        position.estimate(dummy_pd, sampling_frequency_hz)

        # is the result nearly equal to zero?
        np.testing.assert_array_almost_equal([0, 0, 0], position.velocity_.iloc[-1], decimal=2)

        # is the result nearly the same as forward integration?
        np.testing.assert_array_almost_equal([0, 0, 0], scipy.integrate.cumtrapz(dummy_data)[-1])

        # is the result nearly the same as backward integration?
        np.testing.assert_array_almost_equal([0, 0, 0], scipy.integrate.cumtrapz(np.flip(dummy_data))[-1])
