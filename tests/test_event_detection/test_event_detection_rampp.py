...
class TestEventDetectionRampp:
    ...
    def test_min_vel_search_win_size_dummy_data(self):
        """Test if error is raised correctly if windows size matches the size of the input data"""
        dummy_gyr = np.ones((100, 3))
        with pytest.raises(ValueError, match=r"The value chosen for min_vel_search_win_size is too large*"):
            _detect_min_vel(dummy_gyr, dummy_gyr.size)

    def test_valid_min_vel_search_win_size(self, healthy_example_imu_data, healthy_example_stride_borders):
        """Test if error is raised correctly on too large min_vel_search_win_size"""
        data_left = healthy_example_imu_data["left_sensor"]
        data_left.columns = BF_COLS
        stride_list_left = healthy_example_stride_borders["left_sensor"]
        ed = RamppEventDetection(min_vel_search_win_size=5000)
        with pytest.raises(ValueError, match=r"The value chosen for min_vel_search_win_size is too large*"):
            ed.detect(data_left, 204.8, stride_list_left)
